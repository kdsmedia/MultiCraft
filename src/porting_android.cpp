...
/* for it right now */
extern "C" {
	JNIEXPORT void JNICALL Java_com_altomedia_multicraft_GameActivity_putMessageBoxResult(
			JNIEnv *env, jclass j_this, jstring text)
	{
		errorstream << "Java_com_altomedia_multicraft_GameActivity_putMessageBoxResult got: "
...
void initAndroid()
{
	...
	nativeActivity = findClass("com/altomedia/multicraft/GameActivity");
	if (nativeActivity == 0) {
		errorstream <<
